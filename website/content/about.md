---
name: Phan Ngọc Bảo
role: Marketing Specialist
avatar: /uploads/avatar.svg
tagline: Mình viết về Marketing mỗi ngày, từ chiến lược thương hiệu đến những
  thử nghiệm thực chiến.
email: phanngocbao.vn@gmail.com
phone: ""
socials:
  - label: Facebook
    url: https://facebook.com
  - label: LinkedIn
    url: https://linkedin.com
heroEyebrow: Hồ sơ · Phan Ngọc Bảo
heroTitle: This is me
heroImage: https://picsum.photos/seed/ngbao-profile-hero/2000/1300
heroPortrait: /uploads/virtual-headshot-69d7f8ecc10ddab62b0d5c75.jpg
lead: Digital Marketing & Business Leader |  12+ years scaling consumer
  platforms across Vietnam’s retail, fintech, edtech sectors
stats:
  - number: 12+
    label: Năm kinh nghiệm quản lý
    percent: 80
  - number: 50+
    label: Dự án & chiến dịch
    percent: 90
  - number: 5+
    label: Ngành đã đồng hành
    percent: 60
  - number: "1"
    label: Bài viết mỗi ngày
    percent: 100
journey:
  - period: 2015 – 2018
    role: R&D Manager / Head of AI
    place: VTC Intecom
    text: Mình dẫn dắt chiến lược sản phẩm và kiếm tiền cho mảng game casual và
      mobile trên nhiều nền tảng. Công việc là cầu nối giữa đội kỹ thuật và đội
      kinh doanh, dùng big data để ra quyết định, góp phần tạo doanh thu hơn 2
      triệu đô mỗi tháng và mở rộng hệ sinh thái game lên 13 triệu người dùng.
    takeaway: Sản phẩm sống lâu khi giải đúng bài toán kinh doanh, không chỉ chạy
      theo công nghệ.
  - period: 2018 – 2020
    role: Co-Founder / Strategic Growth Leader
    place: WeFit
    text: Mình thiết kế và chạy chiến lược marketing quy mô toàn quốc cho nền tảng,
      xây hệ thống hơn 2000 đối tác ngành lifestyle, dẫn dắt đội marketing và
      sản phẩm khoảng 50 người, quản ngân sách hơn 50 nghìn đô mỗi tháng trên
      Google và Meta. Bọn mình tạo ra một trào lưu gym mới, cho người dùng trải
      nghiệm nhiều phòng tập thay vì chỉ một.
    takeaway: Tăng trưởng nhanh mà thiếu nền tảng vận hành thì rất mong manh.
  - period: 2020 – 2022
    role: Head of Product / Head of Growth
    place: Solvee (Topica Edtech Group)
    text: Mình phụ trách chiến lược sản phẩm và tăng trưởng, đưa app học bằng AI lên
      300 nghìn người dùng hoạt động hàng tháng vào giai đoạn đỉnh. Làm growth
      toàn phễu, tối ưu hành trình và tỷ lệ chuyển đổi cùng đội tech và data,
      giữ vị trí top 2 ngành giáo dục trên App Store và Google Play.
    takeaway: Dữ liệu chỉ có giá trị khi biến được thành quyết định.
  - period: 2022 – 2025
    role: Co-Founder / Regional Director
    place: Mihanoi
    text: Mình thúc đẩy tăng trưởng kinh doanh và độ phủ thương hiệu trên cả nước,
      mở rộng vận hành thương mại điện tử ở phía Nam gồm sàn, logistics và
      performance marketing, xây mạng lưới đối tác phủ toàn miền Nam, hỗ trợ GMV
      khoảng 4 triệu đô.
    takeaway: Thị trường mỗi vùng mỗi khác, đừng bê nguyên công thức cũ.
  - period: Từ 2024
    role: Investor
    place: GW
    text: GW là công ty chuyên về mảng MMO. Đội ngũ xây hệ thống affiliate trên
      Shopee và TikTok thông qua Digital Marketing.
    takeaway: Tư duy hệ thống dùng được cả khi mình đứng ở vai nhà đầu tư.
  - period: Từ 2025
    role: Speaker / Mentor
    place: Viện Đào tạo Quốc tế, ĐH Kinh tế Quốc dân
    text: Mình tham gia chương trình mentoring một kèm một cho sinh viên, chia sẻ
      những gì đã trải qua trong nghề.
    takeaway: Dạy lại người khác là cách mình học sâu nhất.
skills:
  - title: Chiến lược thương hiệu
    desc: Định vị, thông điệp và kiến trúc thương hiệu rõ ràng, nhất quán.
    level: 90
  - title: Performance & đo lường
    desc: Tối ưu hiệu suất dựa trên dữ liệu, mỗi đồng chi ra đều biết mang lại gì.
    level: 85
  - title: Content & storytelling
    desc: Biến thông điệp khô khan thành câu chuyện người ta muốn nhớ và chia sẻ.
    level: 88
  - title: Thương hiệu cá nhân
    desc: Giúp người làm marketing tự xây dựng tiếng nói riêng, đáng tin cậy.
    level: 75
toolkit:
  - group: Quảng cáo & Performance
    items:
      - Meta Ads
      - Google Ads
      - TikTok Ads
      - GDN
  - group: Phân tích & Đo lường
    items:
      - GA4
      - Google Tag Manager
      - Looker Studio
      - UTM
  - group: Content & SEO
    items:
      - Search Console
      - Ahrefs
      - Notion
      - WordPress
  - group: CRM & Automation
    items:
      - Email automation
      - CRM
      - Zapier / Make
  - group: Sáng tạo
    items:
      - Figma
      - Canva
      - CapCut
  - group: AI hỗ trợ
    items:
      - ChatGPT
      - Claude
      - Midjourney
life:
  - image: https://picsum.photos/seed/ngbao-life-read/1200/1500
    video: ""
    title: Đọc và ghi chép
    text: Mình đọc mỗi ngày, chủ yếu về kinh doanh, tâm lý và công nghệ, rồi ghi lại
      thành ghi chú để dùng dần.
  - image: https://picsum.photos/seed/ngbao-life-travel/1200/1500
    video: /uploads/demo-life.mp4
    title: Đi và quan sát
    text: Mỗi chuyến đi là một lần quan sát thị trường và con người ở một góc khác.
  - image: https://picsum.photos/seed/ngbao-life-coffee/1200/1500
    video: ""
    title: Cà phê và trò chuyện
    text: Mình thích những buổi cà phê dài, nghe người khác kể về công việc và cuộc
      sống của họ.
  - image: https://picsum.photos/seed/ngbao-life-gym/1200/1500
    video: ""
    title: Rèn thể lực
    text: Giữ sức khỏe để đủ năng lượng theo đuổi mấy thứ dài hơi.
ctaTitle: Cùng xây điều gì đó đáng nhớ?
ctaText: Nếu bạn đang tìm người đồng hành cho bài toán thương hiệu hoặc
  performance, mình rất sẵn lòng trò chuyện.
showcaseTitle: Mỗi dự án là một hành trình
showcaseSubtitle: Khoảnh khắc & câu chuyện
showcaseImages:
  - https://picsum.photos/seed/ngbao-show1/2200/1300
  - https://picsum.photos/seed/ngbao-show2/2200/1300
  - https://picsum.photos/seed/ngbao-show3/2200/1300
---

Xin chào, mình là **Phan Ngọc Bảo**. Trang này là nơi mình ghi lại, hệ thống hóa và chia sẻ những gì học được mỗi ngày trong nghề Marketing, mong rằng có điều gì đó hữu ích cho bạn.
